...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    header("Location: manage_sources.php");
    exit;
}
...
<head>
  ...
  <style>
    ...
    .id-chip { display:inline-flex; align-items:center; gap:.25rem; padding:.15rem .5rem; border:1px solid var(--tblr-border-color);
               border-radius:999px; font-size:.8rem; }
    /* Mobile card-like rows */
    @media (max-width: 576px){
      .table { display:none; }              /* 隐藏表格，改用卡片列表 */
              ...
              <tbody>
                <?php foreach ($rows as $row): ?>
                  <tr>
                    ...
                    <td>
                      <input type="text" name="names[]" class="form-control" value="<?= h($row['name']) ?>" placeholder="购买途径名称">
                    </td>
                  </tr>
                <?php endforeach; ?>
                <!-- 新增行 -->
                <tr>
                  ...
                  <td>
                    <input type="text" name="names[]" class="form-control" placeholder="新增购买途径">
                  </td>
                </tr>
              </tbody>
            </table>
